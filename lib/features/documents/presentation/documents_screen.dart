...
class _DocumentsScreenState extends ConsumerState<DocumentsScreen> {
  ...
  Widget _buildDocExpansion(DocChecklist doc, Color catColor, bool isDark, bool allowDelete) {
    ...
    final isLocalFile = doc.fileUrl.startsWith('local:');
    String localId = '';
    ...
    if (isLocalFile) {
      ...
      localId = parts[0];
      if (parts.length > 1) {
        displayFileName = parts.sublist(1).join('|');
      } else {
        displayFileName = 'File Terlampir';
      }
    }
      ...
      child: Column(
        ...
        children: [
          ...
          _buildExpansionField(
            ...
            color: catColor,
            maxLines: 3,
            onSave: () => ref.read(documentsControllerProvider.notifier).updateNotes(doc.id, notesCtrl.text),
          ),
          const SizedBox(height: 12),
          // Link file / File Lokal
          if (isLocalFile)
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                  ...
                  child: Row(
                    children: [
                      ...
                      IconButton(
                        ...
                        onPressed: () => indexedDBService.downloadFile(localId),
                        tooltip: 'Unduh File',
                        padding: EdgeInsets.zero,
                        constraints: const BoxConstraints(),
                      ),
                      ...
                      IconButton(
                        ...
                        onPressed: () => _confirmDeleteFile(doc, localId),
                        tooltip: 'Hapus File',
                        padding: EdgeInsets.zero,
                        constraints: const BoxConstraints(),
                      ),
            ...
            _buildExpansionField(
              ...
              maxLines: 1,
              onSave: () => ref.read(documentsControllerProvider.notifier).updateFileUrl(doc.id, fileCtrl.text),
              actionButton: ElevatedButton.icon(
                onPressed: () async {
                  ...
                  if (result != null && result.files.isNotEmpty) {
                    ...
                    if (file.bytes != null) {
                      final fileId = 'local_${DateTime.now().millisecondsSinceEpoch}';
                      ...
                      final url = 'local:$fileId|${file.name}';
                      ref.read(documentsControllerProvider.notifier).updateFileUrl(doc.id, url);
                      _fileUrlControllers[doc.id]?.text = url;
                    }
                  }
                },
                style: ElevatedButton.styleFrom(
          ...
          if (doc.fileUrl.isNotEmpty && !isLocalFile) ...[
            const SizedBox(height: 12),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
              decoration: BoxDecoration(
              ...
              child: Row(
                children: [
                  ...
                  TextButton.icon(
                    onPressed: () {
                      html.window.open(doc.fileUrl, '_blank');
                    },
          ...
          children: [
            ...
            ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                ...
                elevation: 0,
                padding: EdgeInsets.symmetric(horizontal: 12, vertical: maxLines > 1 ? 22 : 14),
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
              ),
  ...
  void _confirmDeleteFile(DocChecklist doc, String localId) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: const Color(0xFF1E293B),
        ...
        content: Text('Berkas yang diupload untuk "${doc.title}" akan dihapus permanen.', style: const TextStyle(fontSize: 13, color: Color(0xFF94A3B8))),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Batal')),
          ElevatedButton(
            onPressed: () {
